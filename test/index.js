...
test("pdf.create(html[, options]).toFile([filename, ]callback)", function (t) {
  ...
  pdf.create(html).toFile(function (err, pdf) {
    ...
    t.assert(
      ...
      `toFile(callback) returns {filename: '${pdf.filename}'} as second cb argument`
    );
  ...
  pdf.create(html).toFile(file, function (err, pdf) {
    ...
    t.assert(
      pdf.filename === file,
      `toFile(filename, callback) returns {filename: '${pdf.filename}'} as second cb argument`
    );
...
test("pdf.create(html).toBuffer(callback)", function (t) {
  ...
  pdf.create(html).toBuffer(function (err, pdf) {
    ...
    t.assert(
      Buffer.isBuffer(pdf),
      "toBuffer(callback) returns a buffer instance as second cb argument"
    );
    t.assert(
      /^%PDF-1.4/.test(pdf.slice(0, 100).toString()),
      "the PDF buffer has a PDF Header"
    );
  });
});
...
test("window.callPhantom renders page", function (t) {
  ...
  var callbackHtml = fs.readFileSync(
  ...
  pdf
    .create(callbackHtml, { renderDelay: "manual" })
    .toFile(file, function (err, pdf) {
      ...
      t.assert(
        time > 1000 && time < 2000,
        "rendered in response to callPhantom"
      );
      t.assert(fs.existsSync(file), "writes the file to the given destination");
    });
});
...
test("pdf.create(html[, options]).toStream(callback)", function (t) {
  ...
  pdf.create(html).toStream(function (err, stream) {
    ...
    t.assert(
      ...
      "toStream(callback) returns a fs.ReadStream as second cb argument"
    );
    ...
    stream.on("end", function () {
      t.assert(
        ...
        "toStream returns a working readable stream"
      );
...
test("allows invalid phantomPath", function (t) {
  ...
  pdf.create(html, options).toFile(filename, function (error, pdf) {
...
test("allows custom page and footer options", function (t) {
  ...
  pdf.create(html, options).toFile(filename, function (error, pdf) {
    ...
    t.assert(
      ...
      "Returns the filename from the phantom script"
    );
    t.assert(
      fs.existsSync(pdf.filename),
      "Saves the pdf with a custom page size and footer"
    );
  });
});
...
test("allows different header and footer for first page", function (t) {
  ...
  var enrichedHtml = fs.readFileSync(
    path.join(__dirname, "multiple-pages.html"),
    "utf8"
  );
...
test("load external css", function (t) {
  ...
  var enrichedHtml = fs.readFileSync(
    path.join(__dirname, "external-css.html"),
    "utf8"
  );
  var filename = path.join(__dirname, "external-css.pdf");
  pdf.create(enrichedHtml).toFile(filename, function (error, pdf) {
    t.error(error);
    t.assert(
      pdf.filename === filename,
      "Returns the filename from the phantom script"
...
test("load external js", function (t) {
  ...
  var enrichedHtml = fs.readFileSync(
    path.join(__dirname, "external-js.html"),
    "utf8"
  );
