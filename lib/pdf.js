...
function PDF(html, options) {
  ...
  assert(
    ...
    "html-pdf: Can't create a pdf without an html string"
  );
...
PDF.prototype.toStream = function PdfToStream(callback) {
  this.exec(function (err, res) {
    ...
    var dimensions = { height: res.height, width: res.width };

    callback(null, stream, dimensions);
  });
};
...
PDF.prototype.toFile = function PdfToFile(filename, callback) {
  assert(
    arguments.length > 0,
    "html-pdf: The method .toFile([filename, ]callback) requires a callback."
  );
...
PDF.prototype.exec = function PdfExec(callback) {
  ...
  var timeout = setTimeout(function execTimeout() {
    respond(
      null,
      new Error(
        "html-pdf: PDF generation timeout. Phantom.js script did not exit."
      )
    );
